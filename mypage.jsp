...
<header id="responsive">
 ...
 %> 알림
            ...
            <section>
                ...
                <div id="webnav">메인 > 마이페이지 > 판매상품</div>
                <div id="leftSide">
            		...
            		String[] orderbySelected = new String[4];
            		if (orderby == null) { orderby = "new"; }
            		if (orderby.equals("hits")) { orderbySelected[1] = "selected"; }
            		else if (orderby.equals("lowprice")) { orderbySelected[2] = "selected"; }
            		else if (orderby.equals("highprice")) { orderbySelected[3] = "selected"; }
            		...
            		String jsqlcnt = "select count(*) from tradepost where m_id = ?";
                    ...
                    <div id="mypageMenu">
                        <ul>
                            <li id="Munu1" class="nowPage" onClick="location.href='mypage.jsp?m_id=<%=m_id%>'" style="cursor:pointer;font-size:16px;"><a href="#a">판매상품</a></li>
                            <li id="Munu2" onClick="location.href='mypage_manage.jsp?m_id=<%=m_id%>'" style="cursor:pointer;font-size:16px;"><a href="mypage_manage.jsp?m_id=<%=m_id%>">거래관리</a></li>
                            <li id="Munu3" onClick="location.href='mypage_likeProduct.jsp?m_id=<%=m_id%>'" style="cursor:pointer;font-size:16px;"><a href="mypage_likeProduct.jsp?m_id=<%=m_id%>">찜한상품</a></li>
                            <li id="Munu4" onClick="location.href='mypage_likeContent.jsp?m_id=<%=m_id%>'" style="cursor:pointer;font-size:16px;"><a href="mypage_likeContent.jsp?m_id=<%=m_id%>">좋아하는컨텐츠</a></li>
                            <li id="Munu5" onClick="location.href='mypage_modify.jsp?m_id=<%=m_id%>'" style="cursor:pointer;font-size:16px;"><a href="mypage_modify.jsp">프로필수정</a></li>
                        </ul>
                    </div>
					<% } else { %>
                    <div id="mypageMenu">
                ...
                <div id="rightSide">
                    ...
                    <div id="topbar">
                        <!-- 제목 -->
                        <div id="title"><%=nickname%>님의 판매상품</div>
                        <!-- 상품정렬 -->
                        <div id="productOrder">
                        	<form action="#" name="orderForm">
                            <select name="orderSelect" onchange="order()">
								<option value="new" <%=orderbySelected[0]%>>최신순</option>
								<option value="hits" <%=orderbySelected[1]%>>조회순</option>
								<option value="lowprice" <%=orderbySelected[2]%>>저가순</option>
								<option value="highprice" <%=orderbySelected[3]%>>고가순</option>
							</select>
							</form>
                        </div>
                    </div>
					...
					String jsqlcontent = "select * from tradepost where m_id = ? order by tp_hits desc, tp_id desc";
					if (orderby.equals("hits")) { jsqlcontent = "select * from tradepost where m_id = ? order by tp_hits desc, tp_id desc"; }
					else if (orderby.equals("lowprice")) { jsqlcontent = "select * from tradepost where m_id = ? order by tp_goodsprice asc, tp_id desc"; }
					else if (orderby.equals("highprice")) { jsqlcontent = "select * from tradepost where m_id = ? order by tp_goodsprice desc, tp_id desc"; }
					else { jsqlcontent = "select * from tradepost where m_id = ? order by tp_id desc"; }
					...
					if(!rscontent.wasNull()) {
						...
						while(rscontent.next()) {
							...
							String tp_titleTemp = rscontent.getString("tp_title");
							String tp_title = "";
							String tp_img = rscontent.getString("tp_goodsimg");
							String tp_date = rscontent.getString("tp_date");
							int tp_hits = rscontent.getInt("tp_hits");
							int tp_priceTemp = rscontent.getInt("tp_goodsprice");
					...
					%>
					 <!--상품카드1-->
                        <div class="productCard">
                            <div id="imgmom">
                                <a href="productDetail.jsp?tp_id=<%=tp_id%>"><img src="<%=tp_img%>"></a>
                            </div>
                    ...
                    <div id="paging">
						<ol class="paging">
							<li class="first"><a href="mypage.jsp?m_id=<%=m_id%>&paging=0">첫페이지</a></li>
							<li class="prev"><a href="mypage.jsp?m_id=<%=m_id%>&paging=<%=prevpage%>">이전페이지</a></li>
							<% 
							for (int j=startpage ; j<= endpage ; j++) {
								if (j == paging + 1) {
									...
									%>
									<li class="on"><a href="mypage.jsp?m_id=<%=m_id%>&paging=<%=j - 1%>"><%=j %></a></li>
									<%
								}
							}
							%>
							<li class="next"><a href="mypage.jsp?m_id=<%=m_id%>&paging=<%=nextpage%>">다음페이지</a></li>
							<li class="last"><a href="mypage.jsp?m_id=<%=m_id%>&paging=<%=endpage-1%>">마지막페이지</a></li>
						</ol>
					</div>
		...
		<script>
			function order(){//정렬 순서에 따라 링크 이동
				...
				var orderlink;
				if(order=="new")
					orderlink="mypage.jsp?m_id=<%=m_id%>&orderby=new"; //최신순 링크
				if(order=="hits")
					orderlink="mypage.jsp?m_id=<%=m_id%>&orderby=hits";	//조회수순 링크
				if(order=="lowprice")
					orderlink="mypage.jsp?m_id=<%=m_id%>&orderby=lowprice";	//저가순 링크
				if(order=='highprice')
					orderlink="mypage.jsp?m_id=<%=m_id%>&orderby=highprice";	//고가순 링크
