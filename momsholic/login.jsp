...
<header id="responsive">
 ...
 %> 알림
			...
			<!--로그인			-->
			<div id="login">
				<h1>맘스홀릭 로그인</h1><br><br><br>
				
				<div id="input">
					<form name="login" method="post" action="./userLogin">
						<div id="input_id">아이디:</div>
						<input type="text" name="id" id="loginID" class="loginInput">
						<br><br>
						<div id="input_pw">패스워드:</div>
						<input type="password" name="password" onkeydown="onEnterSubmit()" id="loginPW" class="loginInput">
							<div id="loginText"></div>
						<div id="buttonLogin">
							<input type="image" src="images/button/login.png" value="submit" align="adsmiddle">
						</div>
							
						<div id="buttonInsert">
							<a href="register.jsp"><img src="images/button/register.png"></a>
						</div>
					</form>
				</div>
		...
		if(session.getAttribute("loginIDData") != null) {
			loginIDData = (String) session.getAttribute("loginIDData");
		}
		...
		if(messageType == "1") {
			...
			%>
			<script>
			$("#loginPW").css("background-color","#FFCECE");
			$("#loginText").text("비밀번호를 입력해주세요.");
			$("#loginText").css("color", "red");
			</script>
			<%
			...
			%>
			<script>
			$("#loginText").text("아이디나 비밀번호를 확인해주세요.");
			$("#loginText").css("color", "red");
			</script>
			<%
		}
		if(loginIDData != null) {
			%>
			<script>
			$("#loginID").val('<%=loginIDData%>');
			</script>
			<%
		}
			session.removeAttribute("loginIDData");
