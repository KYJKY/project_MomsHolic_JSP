...
<header id="responsive">
 ...
 %> 알림
            ...
            <!-- 기부/응모 전환탭 -->
            <div id="manuTab">
                <a href="sub_donate.jsp"><div id="donateTab">물품 기부하기</div></a>
                <a href="sub_entry.jsp"><div id="entryTab">응모하기</div></a>
            </div>
			...
			c3.set(yyyy4, MM4, dd4, hh4, mm4, ss4);
			...
			long remainArray[] = new long[cnt3];
			...
			for ( int i = 0 ; i < cnt3 ; i++) {
					...
					int do_priceTemp = Integer.parseInt(rscontent3.getString("do_price"));
					int do_priceTemp2 = Integer.parseInt(rscontent3.getString("do_price"))/10;
					String do_date = rscontent3.getString("do_date");
					String do_price = df4.format(do_priceTemp);
					String do_price2 = df4.format(do_priceTemp2);
					...
					String reshh4 = do_date.substring(11,13);
					String resmm4 = do_date.substring(14,16);
					String resss4 = do_date.substring(17,19);
					String resdate4 = resyy4+resMM4+resdd4+reshh4+resmm4+resss4;
					String today4 = dfdate3.format(c3.getTime());
					...
					beginDate4 = dfdate3.parse(resdate4);
					endDate4 = dfdate3.parse(today4);
			...
			%>
					...
					<div class="entryCard">
						<input type="hidden" id="hiddenremain<%=cnt3-i%>" value="<%=remainArray[i]%>">
						<img src="<%=do_img%>">
						<div class="entryInfo">
							<p class="entryTitle"><%=do_title %></p>
							<p class="priceText">응모 할인가</p>
							<div class="entryPrice"><h2 class="beforePrice"><del><%=do_price %>원</del></h2><h2 class="afterPrice"><%=do_price2 %>원</h2></div>
						</div>
						<div class="entryTime">
							<p class="timeText">응모까지 남은 시간</p>
							<h1 id="remainTime<%=cnt3-i%>" class="remainTime">로딩 중..</h1>
							<a href="#a" id="entryButton<%=cnt3-i%>" class="entryButton">응모하기</a>
						</div>
					</div>
			<%
			}
			...
			<footer>
				<div id="footerimg">
					...
					%>
					<img src="images/logo.png" id="footerLogo" onclick="location.href='sub_donate_write.jsp'" style="cursor:pointer;">
					<%
					} else {
					%>
	...
	<script> 
			...
			$(document).ready(function(){
				...
				for (int i = 0 ; i < cnt3 ; i++) {
				%>
				$("#entryButton<%=(cnt3-i)%>").on('click',function(){
					<%
					if (loginID == null) {
						%>
						...
						$.ajax({
							type: 'POST',
							url: './UserDonateCheckServlet',
							data: {m_id: '<%=loginID%>', do_id: <%=(cnt3-i)%>},
							success: function(result) {
								if(result == 1) {
									alert("해당 상품을 응모하였습니다.");
								}
								else if (result == 0) {
									alert("이미 응모한 상품입니다.");
								} else {
			...
			//응모 남은시간
			var remain = []; //응모 남은시간(초) 나중에 jsp로 받아와서 넣기
			for (var i = 0; i < <%=cnt3%> ; i++) {
				remain[i] = document.getElementById('hiddenremain' + (i+1)).value;
			}
			...
			var timer = setInterval(function() {
					//parseInt() : 정수를 반환
					for (var x = 0 ; x < <%=cnt3%>; x++) {
						if (remain[x] >= 0) {
							$("#entryButton"+(x+1)).css({"display":"block"});
							min[x] = parseInt(remain[x]/60)%60;//분
							sec[x] = remain[x]%60; //초
							hour[x] = parseInt(remain[x]/60/60); //시간 

							document.getElementById("remainTime"+(x+1)).innerHTML =hour[x] + ":" + min[x] + ":" + sec[x] + "";
							remain[x]--;
						}
						//타임아웃 시
						if (remain[x] < 0) {
							document.getElementById("remainTime"+(x+1)).innerHTML = "응모종료";
							$("#entryButton"+(x+1)).css({"display":"none"});
						}
					}
				}, 1000);
		</script>
