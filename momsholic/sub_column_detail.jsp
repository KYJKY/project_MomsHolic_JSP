...
<html>
	<head>
		...
		$(document).ready(function(){
			...
			// DB접속
					...
					Class.forName("org.gjt.mm.mysql.Driver"); 
			 		...
			 		String jsql = "select * from columntbl where co_id = ?";
					...
					String date = rs.getString("co_date").substring(0,16);
					String img = rs.getString("co_img");
					int hits = rs.getInt("co_hits");
					int likes = rs.getInt("co_likes");
					...
					// 조회수 증가
					hits++;
					String jsqlhits = "update columntbl set co_hits = ? where co_id = ?";
					PreparedStatement pstmthits = con.prepareStatement(jsqlhits);
					pstmthits.setInt(1, hits);
					pstmthits.setInt(2, Integer.parseInt(co_id));
					pstmthits.executeUpdate();
					
					String jsqldibsload = "select count(*) from likecolumn where co_id = ? and m_id = ?";
					PreparedStatement pstmtdibsload = con.prepareStatement(jsqldibsload);
					pstmtdibsload.setInt(1,Integer.parseInt(co_id));
		...
		function dibschange() {
			...
			$.ajax({
				type: 'POST',
				url: './LikeColumnCheckServlet',
				data: {co_id:<%=co_id%>, loginID:"<%=loginIDlike%>"},
 ...
 %> 알림
			...
			<section id="column">
				...
				<div class="columnInfo">
					...
					<p class="info">작성자&nbsp;&nbsp;<%=name %>  |  <%=date %>  |  좋아요 <%=likes %>  |  조회수 <%=hits %></p>
				</div>
