...
<header id="responsive">
 ...
 %> 알림
            ...
            <section>
                ...
                <div id="webnav">메인 > 마이페이지 > 찜한상품</div>
                <div id="leftSide">
                    ...
                    <div id="mypageMenu">
                        <ul>
                            ...
                            <li id="Munu3" class="nowPage" style="cursor:pointer;font-size:16px;"><a href="#a">찜한상품</a></li>
                ...
                <div id="rightSide">
                    ...
                    <div id="topbar">
                        <!-- 제목 -->
                        <div id="title"><%=nickname%>님의 찜한상품</div>
                        <!-- 상품정렬 -->
                        <div id="productOrder">
					...
					String jsqlcontent = "select * from wish where m_id = ? order by tp_hits desc, tp_id desc";
					if (orderby.equals("hits")) { jsqlcontent = "select * from wish where m_id = ? order by tp_hits desc, tp_id desc"; }
					else if (orderby.equals("lowprice")) { jsqlcontent = "select * from wish where m_id = ? order by tp_price asc, tp_id desc"; }
					else if (orderby.equals("highprice")) { jsqlcontent = "select * from wish where m_id = ? order by tp_price desc, tp_id desc"; }
					else { jsqlcontent = "select * from wish where m_id = ? order by tp_id desc"; }
					...
					if(!rscontent.wasNull()) {
						...
						while(rscontent.next()) {
							...
							int tp_priceTemp = rscontent.getInt("tp_price");
		...
		<script>
			function order(){//정렬 순서에 따라 링크 이동
				...
				var orderlink;
				if(order=="new")
					orderlink="mypage_likeProduct.jsp?m_id=<%=m_id%>&orderby=new"; //최신순 링크
				if(order=="hits")
					orderlink="mypage_likeProduct.jsp?m_id=<%=m_id%>&orderby=hits";	//조회수순 링크
				if(order=="lowprice")
					orderlink="mypage_likeProduct.jsp?m_id=<%=m_id%>&orderby=lowprice";	//저가순 링크
				if(order=='highprice')
					orderlink="mypage_likeProduct.jsp?m_id=<%=m_id%>&orderby=highprice";	//고가순 링크
