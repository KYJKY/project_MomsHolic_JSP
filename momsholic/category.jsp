...
<body>
	<%
		...
		if (orderby.equals("hits")) {
			...
			orderbySelected[3] = "selected";
		} else {
			orderbySelected[0] = "selected";
		}
		...
		String ctg_name = "";
		if (ctgType.equals("all")) {
			ctg_name = "전체";
		} else if (ctgType.equals("clothes")) {
			ctg_name = "의류";
		} else if (ctgType.equals("shoes")) {
			ctg_name = "신발";
		} else if (ctgType.equals("book")) {
			ctg_name = "도서";
		} else if (ctgType.equals("toy")) {
			ctg_name = "장난감/완구";
		} else if (ctgType.equals("etc")) {
			ctg_name = "기타";
		}
	%>
	<div id="wrap">
				...
				<ul>
					...
					<li>
						<form>
							...
							<div id="searchButton">
								<a href="#"><img src="images/button/search.png"></a>
							</div>
						</form>
					</li>
 ...
 %> 알림
		...
		<section id="category">
			<div id="categoryInfo">
				...
				<div id="categoryName">중고물품 전체보기</div>
				<div id="categoryOrder">
			...
			<div id="productGroup">
				...
				<%
					...
					/* 카테고리 전체일 경우 */
					if (ctgType.equals("all")) {
						String jsqlCount = "select count(*) from tradepost";
						...
						String jsql = "";
						if (orderby.equals("hits")) {
							jsql = "select * from tradepost order by tp_hits desc, tp_id desc";
						} else if (orderby.equals("lowprice")) {
							jsql = "select * from tradepost order by tp_goodsprice asc, tp_id desc";
						} else if (orderby.equals("highprice")) {
							jsql = "select * from tradepost order by tp_goodsprice desc, tp_id desc";
						} else {
							jsql = "select * from tradepost order by tp_id desc";
						}
				...
				<%
					cursor++;
						...
						String jsqlCount = "select count(*) from tradepost where tp_goodsctg = ?";
						PreparedStatement pstmtCount = con.prepareStatement(jsqlCount);
						...
						String jsql = "";
						if (orderby.equals("hits")) {
							jsql = "select * from tradepost where tp_goodsctg = ? order by tp_hits desc, tp_id desc";
						} else if (orderby.equals("lowprice")) {
							jsql = "select * from tradepost where tp_goodsctg = ? order by tp_goodsprice asc, tp_id desc";
						} else if (orderby.equals("highprice")) {
							jsql = "select * from tradepost where tp_goodsctg = ? order by tp_goodsprice desc, tp_id desc";
						} else {
							jsql = "select * from tradepost where tp_goodsctg = ? order by tp_id desc";
						}
						PreparedStatement pstmt = con.prepareStatement(jsql);
		...
		<div id="paging">
			<ol class="paging">
				<li class="first"><a
					href="category.jsp?ctg=<%=ctgType%>&paging=0">첫페이지</a></li>
				<li class="prev"><a
					href="category.jsp?ctg=<%=ctgType%>&paging=<%=prevpage%>">이전페이지</a></li>
				<%
				...
				%>
				<li class="on"><a
					href="category.jsp?ctg=<%=ctgType%>&paging=<%=j - 1%>"><%=j%></a></li>
				<%
					}
					}
				%>
				<li class="next"><a
					href="category.jsp?ctg=<%=ctgType%>&paging=<%=nextpage%>">다음페이지</a></li>
				<li class="last"><a
					href="category.jsp?ctg=<%=ctgType%>&paging=<%=endpage - 1%>">마지막페이지</a></li>
			</ol>
		</div>
	...
	<script>
			function order(){//정렬 순서에 따라 링크 이동
				...
				var orderlink;
				if(order=="new")
					orderlink="category.jsp?ctg=<%=ctgType%>&orderby=new"; //최신순 링크
				if(order=="hits")
					orderlink="category.jsp?ctg=<%=ctgType%>&orderby=hits";	//조회수순 링크
				if(order=="lowprice")
					orderlink="category.jsp?ctg=<%=ctgType%>&orderby=lowprice";	//저가순 링크
				if(order=='highprice')
					orderlink="category.jsp?ctg=<%=ctgType%>&orderby=highprice"; //고가순 링크
