...
<html>
	...
	<%
		...
		if (orderby.equals("old")) { orderbySelected[1] = "selected"; }
		else if (orderby.equals("likes")) { orderbySelected[2] = "selected"; }
		else if (orderby.equals("hits")) { orderbySelected[3] = "selected"; }
		...
		if (pagingTemp != null)
 		...
 		String jsqlCount = "select count(*) from columntbl";
 ...
 %> 알림
		...
		<!--서브 키이미지, 사이트 경로-->
			...
			<section>
				<div id="topList">
					...
					<form action="#" name="orderForm">
						<select name="orderSelect" onchange="order()" style="display:none;">
				...
				<%
				String jsql2 = "";
					if (orderby.equals("hits")) { jsql2 = "select * from columntbl order by co_hits desc, co_id desc"; }
					else if (orderby.equals("likes")) { jsql2 = "select * from columntbl order by co_likes desc, co_id desc"; }
					else if (orderby.equals("old")) { jsql2 = "select * from columntbl order by co_id asc"; }
					else { jsql2 = "select * from columntbl order by co_id desc"; }
					PreparedStatement pstmt12 = con.prepareStatement(jsql2);
					ResultSet rs12 = pstmt12.executeQuery();
					
					if(!rs12.wasNull()) {
						for(int i = 0; i < paging * 10; i++) { rs12.next(); }
						int cursor = 0;
							
						while(rs12.next()) {
							String co_id = rs12.getString("co_id");
							String co_titleTemp = rs12.getString("co_title");
							String co_title = "";
							String co_content = "";
							String co_img = rs12.getString("co_img");
							String co_name = rs12.getString("co_name");
							String co_date = rs12.getString("co_date").substring(0,16);
							int co_hits = rs12.getInt("co_hits");
							int co_likes = rs12.getInt("co_likes");
							String co_contentTemp = rs12.getString("co_content").replaceAll("&nbsp;", " ").replaceAll("&lt;", "<").replaceAll("&gt;", ">").replaceAll("<br>", " ");
					...
					<div class="columnCard">
						<a href="sub_column_detail.jsp?co_id=<%=co_id%>"><img src="<%=co_img%>"></a>
						<div class="columnInfo">
							<a href="sub_column_detail.jsp?co_id=<%=co_id%>"><h1 class="title"><%=co_title %></h1></a>
							<a href="sub_column_detail.jsp?co_id=<%=co_id%>"><p class="preview"><%=co_content %></p></a>
							<div class="columnSubinfo">
								<div class="name_date">작성자 <%=co_name %>  | <%=co_date %></div>
								<div class="hits_like">조회수 <b class="hits"><%=co_hits %></b> | 좋아요 <b class="like"><%=co_likes %></b></div>
							</div>
						</div>
					</div>
					<%
			...
			<div id="paging">
				<ol class="paging">
					<li class="first"><a href="sub_column.jsp?paging=0">첫페이지</a></li>
					<li class="prev"><a href="sub_column.jsp?paging=<%=prevpage%>">이전페이지</a></li>
					<% 
					for (int j=startpage ; j<= endpage ; j++) {
						if (j == paging + 1) {
							...
							%>
							<li class="on"><a href="sub_column.jsp?paging=<%=j - 1%>"><%=j %></a></li>
							<%
						}
					}
					%>
					<li class="next"><a href="sub_column.jsp?paging=<%=nextpage%>">다음페이지</a></li>
					<li class="last"><a href="sub_column.jsp?paging=<%=endpage-1%>">마지막페이지</a></li>
				</ol>
				<%
				if (session.getAttribute("loginSession") != null) {
					loginID = (String) session.getAttribute("loginSession");
					if (loginID.equals("admin")) {
						%>
							<a href="sub_column_write.jsp" id="writeButton">칼럼 작성하기</a>
						<%
					}
				}
				%>
