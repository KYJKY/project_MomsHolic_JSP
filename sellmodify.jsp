...
<html>
	...
	<%	 
		...
		String content = rs1.getString("tp_content").replaceAll("&nbsp;", " ").replaceAll("&lt;", "<").replaceAll("&gt;", ">").replaceAll("<br>", "\n");
		...
		int price = rs1.getInt("tp_goodsprice");
		...
		String[] ctgselected = new String [5];
		if (ctg.equals("clothes")) { ctgselected[0] = "selected"; }
		else if (ctg.equals("shoes")) { ctgselected[1] = "selected"; }
		else if (ctg.equals("book")) { ctgselected[2] = "selected"; }
		else if (ctg.equals("toy")) { ctgselected[3] = "selected"; }
		else if (ctg.equals("etc")) { ctgselected[4] = "selected"; }
		
		String[] conchecked = new String [3];
		if (conditionTemp.equals("new")) { conchecked[0] = "checked"; }
		else if (conditionTemp.equals("almost")) { conchecked[1] = "checked"; }
		else if (conditionTemp.equals("used")) { conchecked[2] = "checked"; }
		
		String[] dealchecked = new String [3];
		if (dealingTemp.equals("directDealing")) { dealchecked[0] = "checked"; }
		else if (dealingTemp.equals("deliveryDealing")) { dealchecked[1] = "checked"; }
		else if (dealingTemp.equals("bothDealing")) { dealchecked[2] = "checked"; }
		
		String[] feechecked = new String [2];
		if (deliveryfeeTemp.equals("FeeInclusion")) { feechecked[0] = "checked"; }
		else if (deliveryfeeTemp.equals("collectible")) { feechecked[1] = "checked"; }
		...
		String img1url = "images/sell/preview.png";
		String img2url = "images/sell/preview.png";
		String img3url = "images/sell/preview.png";
		String img4url = "images/sell/preview.png";
	%>
		<div id="wrap">
			<!--헤더-->
		<header id="header">
...
<header id="responsive">
 ...
 %> 알림
			...
			<!--판매하기  -->
			<section>
				<form name="goods" method="post" action="sellmodifyOK.jsp?tp_id=<%=tp_id %>" enctype="multipart/form-data">
				<div id="selltop">
					<p>상품등록</p>
				</div>
				<div id="sellInfo">
					<table>
						<tr style="height:200px">
							<td class="col1">상품사진</td>
							<td class="col2">
							<% if (img1 != null) { img1url = "upload/"+img1; }
							if (img2 != null) { img2url = "upload/"+img2; } 
							if (img3 != null) { img3url = "upload/"+img3; } 
							if (img4 != null) { img4url = "upload/"+img4; } %>
								<div class="imgpreview">
								<label for="filename1"><img src="<%=img1url %>" id="previewImg1" class="imgPointer"></label>
								<input type="file" accept="images/*" name="filename1" class="uploadHidden" id="filename1" style="display:none;" onchange="readImg1(this);">
								</div>
								<div class="imgpreview">
								<label for="filename2"><img src="<%=img2url %>" id="previewImg2" class="imgPointer"></label>
								<input type="file" accept="images/*" name="filename2" class="uploadHidden" id="filename2" style="display:none;" onchange="readImg2(this);">
								</div>
								<div class="imgpreview">
								 <label for="filename3"><img src="<%=img3url %>" id="previewImg3" class="imgPointer"></label>
								<input type="file" accept="images/*" name="filename3" class="uploadHidden" id="filename3" style="display:none;" onchange="readImg3(this);">
								</div>
								<div class="imgpreview">
								<label for="filename4"><img src="<%=img4url %>" id="previewImg4" class="imgPointer"></label>
						...
						<tr>
							<td class="col1">판매글 제목</td>
							<td class="col2"><input type="text" class="inputTitle" placeholder="제목을 입력해주세요." name="title" value="<%=title %>" required></td>
						</tr>
						...
						<tr>
							...
							<td class="col2">
								<select class="inputCategory" name="category">
									<option value="clothes" <%=ctgselected[0] %>>의류</option>
									<option value="shoes" <%=ctgselected[1] %>>신발</option>
									<option value="book" <%=ctgselected[2] %>>도서</option>
									<option value="toy" <%=ctgselected[3] %>>장난감/완구</option>
									<option value="etc" <%=ctgselected[4] %>>기타</option>
								</select>
							</td>
						</tr>

						<tr>
							<td class="col1">상품가격</td>
							<td class="col2"><input type="number" class="inputPrice" placeholder="가격을 입력해주세요." min="100" max="10000000" maxlength="7" name="price" oninput="maxLengthCheck(this)" required value="<%=price%>"> 원</td>
						</tr>

						<tr>
							<td class="col1">상품상태</td>
							<td class="col2">
								<input type="radio" id="condition1" name="condition" value="new" checked="<%=conchecked[0] %>">
								<label for="condition1">새상품</label>
								<input type="radio" id="condition2" name="condition" value="almost" checked="<%=conchecked[1] %>">
								<label for="condition2">거의새것</label>
								<input type="radio" id="condition3" name="condition" value="used" checked="<%=conchecked[2] %>">
								<label for="condition3">중고</label>
							</td>
						</tr>
						...
						<tr>
							...
							<td class="col2">
								<input type="radio" id="dealing1" name="dealing" value="directDealing" checked="<%=dealchecked[0] %>">
								<label for="dealing1">직거래</label>
								<input type="radio" id="dealing2" name="dealing" value="deliveryDealing" checked="<%=dealchecked[1] %>">
								<label for="dealing2">택배거래</label>
								<input type="radio" id="dealing3" name="dealing" value="bothDealing" checked="<%=dealchecked[2] %>"> 
								<label for="dealing3">상관없음</label>
							</td>
						</tr>
						...
						<tr>
							...
							<td class="col2">
								<input type="radio" id="deliveryFee" name="deliveryFee" value="FeeInclusion" checked="<%=feechecked[0] %>">
								<label for="dealing1">배송비 포함</label>
								<input type="radio" id="deliveryFee" name="deliveryFee" value="collectible" checked="<%=feechecked[1] %>">
								<label for="dealing2">착불</label>
							</td>
						</tr>

						<tr>
							<td class="col1">상품설명</td>
							<td class="col2"><textarea name="content" rows="10" cols="50" placeholder="상품설명을 입력해주세요." required><%=content %></textarea></td>
						</tr>
					</table>
