...
### 사이트 소개
- 유아용품 중고거래 사이트 Mom's Holic은 유아용품 중고거래와 육아 콘텐츠에 특화된 웹 서비스를 제공한다.
...
<br><br><br>
아쉬웠던 점
- 자바스크립으로 동적인 효과를 많이 못넣은 것에 있다. 간단한 자바스크립트만 사용하여 기존 구현하고 싶던 기능이 실력과의 타협으로 구현할 수 없었다.
- 파일의 효율적인 관리가 부족했다.
- css를 너무 지저분하게 작성했다. 매번 발표기한에 쫓기면서 css파일을 기능별로 나누지도 못했고 반응형 또한 따로 나누지도 못했다. 
- css 선택자를 한정적으로 사용했다. 더 폭넓은 선택자를 사용했더라면 쓸데없는 id와 class를 만들지 않았을 것이다.
- 이 당시 git이라는 것을 몰랐다. 협업이기 때문에 git을 통해 버전관리를 했더라면 프로젝트를 더욱 수월하게 진행했을 것이다. 실제로 파일을 합치면서 생긴 이슈가 많다.
