...
<html>
	<head>
		...
		<link href="css/sub_diary.css" rel="stylesheet" type="text/css">
...
<header id="responsive">
 ...
 %> 알림
            ...
            <!--서브 키이미지, 사이트 경로-->
            <div id="subKeyimage">
				<img src="images/subpage/diary/diary_subimage.jpg">
                <h1>MOM's 일기</h1>
                <h2>옆집 엄마들과 함께 공감해요!</h2>
                <h3>메인 > MOM's 일기</h3>
            </div>
			...
			<section>
				...
				<!-- 다이어리그룹 -->
				<div class="diaryGroup">
				<%
				String jsqlCount = "select count(*) from diary";
				...
				int cntList = cnt/16; // 페이지 수 계산
				int remainder = cnt%16; // 나머지
				...
				String jsql = "select * from diary order by di_id desc";
				PreparedStatement pstmt = con.prepareStatement(jsql);
				ResultSet rs = pstmt.executeQuery();
				
				if(!rs.wasNull()) {
					for(int i = 0; i < paging * 12; i++) { rs.next(); }
					int cursor = 0;
						
					while(rs.next()) {
						String di_id = rs.getString("di_id");
						String di_titleTemp = rs.getString("di_title");
						String di_title = "";
						String di_img = rs.getString("di_img");
						String di_m_id = rs.getString("m_id");
						String di_m_nickname = rs.getString("m_nickname");
						String di_date = rs.getString("di_date").substring(0,16);
						int di_hits = rs.getInt("di_hits");
						int di_comments = rs.getInt("di_comments");
				...
				%>
				<!--상품카드1-->
				<div class="diaryCard" style="cursor:pointer;" onclick="location.href='sub_diary_detail.jsp?di_id=<%=di_id%>'">
						<img src="<%=di_img%>">
						<div class="diaryInfo">
							<h1><%=di_title %></h1>
							<p class="writer"><%= di_m_nickname%> (<%=di_m_id %>)</p>
							<p class="date">조회수<%=di_hits %>  |  댓글<%=di_comments %>  |  <%=di_date %></p>
						</div>
					</div>
				<%
					cursor ++;
			...
			<div id="paging">
				<ol class="paging">
					<li class="first"><a href="sub_diary.jsp&paging=0">첫페이지</a></li>
					<li class="prev"><a href="sub_diary.jsp&paging=<%=prevpage%>">이전페이지</a></li>
					<% 
					for (int j=startpage ; j<= endpage ; j++) {
						if (j == paging + 1) {
							...
							%>
							<li class="on"><a href="sub_diary.jsp&paging=<%=j - 1%>"><%=j %></a></li>
							<%
						}
					}
					%>
					<li class="next"><a href="sub_diary.jsp&paging=<%=nextpage%>">다음페이지</a></li>
					<li class="last"><a href="sub_diary.jsp&paging=<%=endpage-1%>">마지막페이지</a></li>
				</ol>
				<a href="sub_diary_write.jsp" id="writeButton">일기 작성하기</a>
			</div>
